...
- ~~The phone↔watch Data Layer sync has never been observed working end to end (item 7 below).~~
  **CLOSED 31 Aug 2026.** The owner paired a watch and reported that Asr agreed with the phone
  set to Hanafi, which is the one comparison the madhab moves far enough to be unmistakable.
  A per-prayer adjustment was not tried, so `KEY_ADJUSTMENTS` remains unobserved on its own.
